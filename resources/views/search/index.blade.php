...
@section('content')
	...
	<article class="cf bb b--black-10">
		<div class="fl w-100 w-50-ns tc">
			<article class="pa1 pa3-ns tc">
			...
			<article class="pa1 pa3-ns tc">
				...
				<section class="cf w-100 pa2-ns">
					@foreach ($albums->albums->items as $album)
						<article class="fl w-100 w-50-m w-25-ns pa2-ns">
							<div class="aspect-ratio aspect-ratio--1x1">
								<a href="/albums/{{ $album->id }}" class="dim">
									<img style="background-image:url({{ $album->images[1]->url }});"
										 class="db bg-center cover aspect-ratio--object" />
								</a>
							</div>

							<a href="/albums/{{ $album->id }}" class="ph2 ph0-ns pb3 link db dim">
								<h3 class="f5 f4-ns mb0 black-90">{{ $album->name }}</h3>
								<h3 class="f6 f5 fw4 mt2 black-60">{{ $album->artists[0]->name }}</h3>
							</a>
						</article>
